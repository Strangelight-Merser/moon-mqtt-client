...
#define IMPORTED_OPEN_SSL_FUNCTIONS\
  ...
  IMPORT_FUNC(int, SSL_get_verify_mode, (const SSL *ssl))\
  ...
  IMPORT_FUNC(int, SSL_CTX_use_certificate_chain_file, (SSL_CTX *ctx, const char *file))\
...
int strangelight_async_tls_load_openssl(int *major, int *minor, int *fix) {
  void *handle = 0;
...
void *strangelight_async_tls_bio_get_endpoint(BIO * bio) {
  void *data = BIO_get_data(bio);
  moonbit_incref(data);
  return data;
}

void strangelight_async_tls_bio_set_flags(BIO * bio, int flags) {
  return BIO_set_flags(bio, flags);
}

void strangelight_async_tls_bio_set_shutdown(BIO * bio, int flags) {
  return BIO_set_flags(bio, flags);
}
...
void strangelight_async_tls_init_bio_method(
  int (*read)(BIO *, void *, int),
  int (*write)(BIO *, const void *, int)
) {
  bio_method = BIO_meth_new(BIO_TYPE_NONE, "strangelight/async-tls");
...
BIO *strangelight_async_tls_create_bio(void *data) {
...
int strangelight_async_tls_ssl_ctx_is_null(SSL_CTX *ctx) {
  return ctx == 0;
}

SSL_CTX *strangelight_async_tls_client_ctx(int32_t load_default_verify_path) {
...
SSL_CTX *strangelight_async_tls_server_ctx() {
  SSL_CTX *server_ctx = SSL_CTX_new(TLS_server_method());
  SSL_CTX_ctrl(server_ctx, SSL_CTRL_MODE, SSL_MODE_ENABLE_PARTIAL_WRITE, 0);
  return server_ctx;
}

void strangelight_async_tls_ssl_ctx_free(SSL_CTX *ctx) {
  SSL_CTX_free(ctx);
}

int strangelight_async_tls_ssl_ctx_add_root_certificate(
  SSL_CTX *ctx,
...
SSL *strangelight_async_tls_ssl_new(SSL_CTX *ctx, BIO *rbio, BIO *wbio) {
  SSL *ssl = SSL_new(ctx);
  if (!ssl) return ssl;
...
int strangelight_async_tls_ssl_connect(SSL *ssl) {
  return SSL_connect(ssl);
}

int strangelight_async_tls_ssl_set_host(SSL *ssl, const char *host) {
  return SSL_set1_host(ssl, host);
}

int strangelight_async_tls_ssl_set_sni(SSL *ssl, void *host) {
  return SSL_ctrl(ssl, SSL_CTRL_SET_TLSEXT_HOSTNAME, TLSEXT_NAMETYPE_host_name, host);
}

void strangelight_async_tls_ssl_set_verify(SSL *ssl, int verify) {
  SSL_set_verify(ssl, verify ? SSL_VERIFY_PEER : SSL_VERIFY_NONE, 0);
}

int strangelight_async_tls_ssl_accept(SSL *ssl) {
  return SSL_accept(ssl);
}

int strangelight_async_tls_ssl_use_certificate_file(
  SSL *ssl,
  const char *file,
  int type
) {
...
int strangelight_async_tls_ssl_use_private_key_file(
  SSL *ssl,
  const char *file,
  int type
) {
...
int strangelight_async_tls_ssl_read(SSL *ssl, char *buf, int offset, int num) {
  return SSL_read(ssl, buf + offset, num);
}

int strangelight_async_tls_ssl_write(SSL *ssl, char *buf, int offset, int num) {
  return SSL_write(ssl, buf + offset, num);
}

int strangelight_async_tls_ssl_shutdown(SSL *ssl) {
  return SSL_shutdown(ssl);
}

void strangelight_async_tls_ssl_free(SSL *ssl) {
  SSL_free(ssl);
}

X509 *strangelight_async_tls_ssl_get_certificate(SSL *ssl, int32_t is_client) {
  return is_client
    ? SSL_get1_peer_certificate(ssl)
    : SSL_get_certificate(ssl);
}

int32_t strangelight_async_tls_peer_certificate_length(X509 *cert) {
  return i2d_X509_AUX(cert, 0);
}

void strangelight_async_tls_peer_certificate_blit_to(X509 *cert, unsigned char *buf) {
  i2d_X509_AUX(cert, &buf);
}

void strangelight_async_tls_free_peer_certificate(X509 *cert) {
  X509_free(cert);
}

MOONBIT_FFI_EXPORT
void *strangelight_async_tls_hash_server_endpoint_certificate(X509 *cert, uint32_t *len_out) {
...
int32_t strangelight_async_tls_ssl_unique_channel_binding_length(SSL *ssl, int32_t is_client) {
  return is_client ?
    SSL_get_finished(ssl, 0, 0) :
    SSL_get_peer_finished(ssl, 0, 0);
}

void strangelight_async_tls_ssl_unique_channel_binding(
  SSL *ssl,
  void *buf,
  int32_t len,
  int32_t is_client
...
int strangelight_async_tls_ssl_get_error(SSL *ssl, int ret) {
  return SSL_get_error(ssl, ret);
}

uint64_t strangelight_async_tls_peek_error_code() {
  return ERR_peek_error();
}

int strangelight_async_tls_get_error(void *buf) {
  unsigned long code = ERR_get_error();
  ERR_error_string(code, buf);
  return strlen(buf);
}

int strangelight_async_tls_rand_bytes(unsigned char *buf, int num) {
  return RAND_bytes(buf, num);
}

void strangelight_async_tls_SHA1(
  moonbit_bytes_t src,
  int32_t len,
  moonbit_bytes_t dst
) {
...
static int reject_passwd_cb(char *buf, int size, int rwflag, void *userdata) {
  (void)buf;
  (void)size;
  (void)rwflag;
  (void)userdata;
...
void strangelight_async_tls_ssl_ctx_reject_encrypted_keys(SSL_CTX *ctx) {
  SSL_CTX_set_default_passwd_cb(ctx, reject_passwd_cb);
}
...
int strangelight_async_tls_ssl_ctx_check_private_key(SSL_CTX *ctx) {
  return SSL_CTX_check_private_key(ctx);
}

int strangelight_async_tls_ssl_ctx_set_min_proto_version(SSL_CTX *ctx, int version) {
  // SSL_CTX_set_min_proto_version is a macro over SSL_CTX_ctrl on OpenSSL 3.
  return (int)SSL_CTX_ctrl(ctx, SSL_CTRL_SET_MIN_PROTO_VERSION, version, 0);
}

void strangelight_async_tls_ssl_ctx_require_client_certificate(SSL_CTX *ctx) {
  SSL_CTX_set_verify(
    ctx,
...
void strangelight_async_tls_ssl_require_client_certificate(SSL *ssl) {
  SSL_set_verify(
    ssl,
    SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT,
    0
