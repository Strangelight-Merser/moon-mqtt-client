...
| Level | Content |
|---|---|
| Core | Fixed native check/test/build, key raw peers including H01, MQTT5, maintenance, recoverable QoS1, short durable crashes, Mosquitto core |
...
| Release | Integration plus immutable candidate/operator consumers, hostile input, storage faults, measured resources and final evidence inventory |
...
Pull requests run Core. The existing weekly schedule and explicit Release runs
execute heavier acceptance; rolling compatibility remains a separate job.
