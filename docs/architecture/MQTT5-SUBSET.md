...
## Public metadata seam and ownership

The initial public value types are `ProtocolVersion::{Mqtt311,Mqtt5}`, `PublishProperties` (optional message-expiry seconds, response topic, binary correlation data, ordered user properties), `BrokerReason` (numeric code and optional reason string), `PublishReceipt::{Written,Accepted(BrokerReason)}`, and `NegotiatedSettings`. Root compiled these isolated types against the pinned native toolchain; they are not yet wired to runtime. An additive detailed publish operation can expose the receipt while existing `publish` retains its Unit completion contract. Subscription/unsubscription detailed results must likewise expose numeric reasons without changing the meaning of existing success/rejection results.

Admission must snapshot the caller's mutable property array before suspension and freeze expiry once. Outgoing retransmissions use that owned snapshot and the remaining immutable deadline, not a caller-mutated array or a fresh interval. Recovered metadata needs a bounded validated representation in the one concrete store; its bytes count toward durable storage bounds. Changing the unreleased schema version is allowed, but old/unknown schema files must be rejected without mutation; no migration or parallel legacy implementation is approved.

A persisted MQTT5 known-session marker is distinct from the presence of pending outbox rows: all deliveries can be acknowledged while the broker still retains the logical session. Commit session ownership before admitting work. Crash before that commit may cause a conservative unexpected-Session-Present rejection on next open; do not silently reset the broker to conceal the uncertainty.

## Durable metadata budget decision

The byte charge is payload length plus the full canonical property-section length when properties are nonempty. The canonical empty section `b"\x00"` contributes zero metadata bytes. Its fixed framing overhead, like fixed SQLite row/schema overhead, remains covered by the page bound. This same rule applies to both protocol versions and preserves the existing six-byte budget for two three-byte payloads; it is not a schema compatibility path. Admission, aggregate SQL bounds and reopen validation must use the same charge. Nonempty metadata overflow must roll back without changing admitted records. Check record/page/byte bounds before loading and decoding all persisted metadata; admission checks lengths before codec allocation.
