...
Updated: 2026-09-18. Expanded software roadmap: **in_progress**. W1/Q1/H1 host software are done; D1 is in_review; M1 runtime is in_progress. Actual HA/ESP32, registry-install and publication gates remain open.
...
- Latest hosted accepted integration head is `ff2bcc88ee43b93f23ff37002ff74d6f8b2ac70b` (H1); later D1/M1 work requires its own acceptance.
...
## D1 current candidate and remaining acceptance
...
- Root final integrated `scripts/check.sh` atd4f5744 passed: native123, broker25, faults10, scenarios/TCP+mTLS consumers, recovery5, durable4, HA4 and real MQTT5 codec roundtrip. Log `_build/exec-outbox/final-integrated-check.log`. New D1 process checks are wired into local and hosted required suites. Final head CI remains required before D1 done; WS/WSS integration is running in `_build/exec-outbox/final-ws-interop.log`.
...
No recursive delegation. Separate worktrees/builds; root currently owns the default-port broker acceptance slot. M1 may use its own native builds and isolated dynamic-port peers, not simultaneous old full broker suites. Root will release the slot explicitly.
