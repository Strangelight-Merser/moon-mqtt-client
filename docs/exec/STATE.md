...
W1 follow-up: diagnostic head6479bc22 run35360597290 revealed (a) macOS raw peer already received DISCONNECT but close framing raced pending completion, and (b) existing Ubuntu TCP EMQX reconnect ended ProtocolError; WSS step was skipped after (b). Root moved optional framing shutdown after MQTT DISCONNECT request completion, preserving bounded best-effort Close and cancellation; independent Sol review approved. Native72, full existing check, raw WS6 and EMQX WS/WSS4 passed locally (`close-order-{check,raw,interop}.log`). Three existing Session fixture literals gain only `finish_transport: None`; assertions unchanged. New native regression enforces completion-before-close-error. CI WS steps now run after successful build even when another independent test failed, while the job retains any failure. The two prior hosted failures remain unresolved until fresh evidence.
