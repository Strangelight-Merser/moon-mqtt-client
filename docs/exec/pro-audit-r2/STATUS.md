...
| WP2 | Original first failure and old/new 100-trial negative results preserved. Isolated post-write race red/green: 60/60 Unknown before the private fix, 60/60 local success after it; native opposite-timing checks added. | Four other barrier classes and pinned Linux EMQX test remain; Release HOLD. |
...
| WP4 | SHA/layout-verified, read-only LLDB active-coroutine snapshots pass 1/16 worker calibration, 20 scope lifetimes, 20 established reconnects and 20 native publish timeouts on local macOS. | Durable reopen/storage-failure short group and Linux SIGSTOP sampler remain. This counts active registered coroutines only. |
| WP5 | Three injected recovery I/O windows pass local Mac; Core and Integration scenario definitions, separate CI contexts, branch-rules proposal, symlink-aware source inventory, E06 gate and operator-consumer path prepared. | Final frozen package/consumer, Linux CI and Release evidence remain; registry validation waits for actual publication. |
